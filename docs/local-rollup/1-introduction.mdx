...
a local Kubernetes cluster which uses the 0xSS shared sequencer network.
...
### Clone the 0xSS [dev-cluster](https://github.com/0xss-network/dev-cluster/tree/main)

```bash
git clone --branch dusk-3 https://github.com/0xss-network/dev-cluster.git
```
...
We use the [0xSS
dev-cluster](https://github.com/0xss-network/dev-cluster) to setup a local
...
config:
  ...
  sequencer:
    ...
    websocket: wss://rpc.sequencer.dusk-3.devnet.0xss.io/websocket
    rpc: https://rpc.sequencer.dusk-3.devnet.0xss.io
ingress:
...
Create an account on the 0xSS shared sequencer network
...
Navigate to < https://faucet.sequencer.dusk-3.devnet.0xss.io/ > to view the
 sequencer faucet.
